...
traces="/home/staff/lucas/mc723/traces/177.mesa.f2b"
p1="177.mesa.f2b"
...
for j in $(seq 1 1 10) # 2K,4K,8K,16K,32K, 64K,128K,256K,512K,1M
	do
		...
		../../dinero4sbc/dineroIV -informat s -trname mesa_f2b -maxtrace 20 -l1-isize ${i_} -l1-dsize ${d_} -l1-ibsize ${ib_} -l1-dbsize ${db_} > ${out_}
	done
name="mesa"
mv .*.*[$name].* ${name}/
